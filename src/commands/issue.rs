...
pub fn create_issue(
    ...
    println!("✓ Created issue '{}/{}' - {}", project_name, issue_id, title);
...
pub fn show_issue(id: &str) -> Result<()> {
    ...
    let (project_name, issue_id) = if let Some((proj, id)) = id.split_once('/') {
        ...
        return Err(anyhow::anyhow!("Issue ID must be in format 'project-name/001'"));
    };
    ...
    println!("Issue: {}/{} - {}", project_name, issue_id, issue.metadata.title);
...
mod tests {
    ...
    fn test_create_issue() -> Result<()> {
        ...
        env::set_current_dir(temp_dir.path())?;
        let result = create_issue(&project_name, "Fix critical bug", "urgent", Some("v1.0"), Some("bug,critical"));
        env::set_current_dir(&original_dir)?;
    ...
    fn test_edit_issue() -> Result<()> {
        ...
        let result = edit_issue("test-project/001", Some("in-progress"), Some("high"), Some("v2.0"), None);
        env::set_current_dir(&original_dir)?;
    ...
    fn test_sanitize_filename() {
        ...
        assert_eq!(sanitize_filename("Add new feature: authentication"), "add-new-feature-authentication");
