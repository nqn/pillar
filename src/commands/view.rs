...
pub fn status() -> Result<()> {
    ...
    if !active_projects.is_empty() {
        ...
        for project in active_projects {
            ...
            let in_progress = issues.iter().filter(|i| i.metadata.status == Status::InProgress).count();
            println!("  • {} ({} issues in progress)", project.metadata.name, in_progress);
        }
        println!();
    }
    ...
    let completed = all_issues.iter().filter(|i| i.metadata.status == Status::Completed).count();
    let todo = all_issues.iter().filter(|i| i.metadata.status == Status::Todo).count();
    
    println!("{}", "Summary:".bold());
    println!("  Projects: {}", projects.len());
    println!("  Issues: {} total, {} completed, {} todo", total_issues, completed, todo);
...
pub fn board(project_filter: Option<&str>) -> Result<()> {
    ...
    for (status, label) in statuses {
        ...
        println!("{} ({})", format_status_label(&status, label), status_issues.len());
        println!("{}", "─".repeat(40));
