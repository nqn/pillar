    ...
    if entity_type == "all" || entity_type == "milestone" {
        ...
        for project in &projects {
            ...
            for milestone in milestones {
                let matches = milestone.metadata.title.to_lowercase().contains(&query_lower)
        ...
        for project in &projects {
            ...
            for issue in issues {
                let matches = issue.metadata.title.to_lowercase().contains(&query_lower)
                    ...
                    || issue.metadata.tags.iter().any(|t| t.to_lowercase().contains(&query_lower));
                ...
                if matches {
                    ...
                    let project_name = issue.metadata.project.as_deref()
                        .unwrap_or("Unknown");
